...
function do_send(state){
    ...
    request(
        ...
        (err, res, body) => {
            ...
            log_debug(__filename, 'SendPost statusCode ' + res.statusCode);

            if (res.statusCode >= 400){
                log_debug(__filename, 'SendPost 57');
                state.next(new Error(`${__filename} failed with ${res.statusCode}`));
                return;
            }

            log_debug(__filename, 'SendPost 62');
            if (res.statusCode == 200){
                try{
                    log_debug(__filename, 'SendPost 65');
                    state.next(
                        body.error ? new Error(body.error) : null,
                        body.result || {}
                    );
                }catch(ex){
                    log_debug(__filename, 'SendPost 71');
            ...
            log_debug(__filename, 'SendPost 78');
